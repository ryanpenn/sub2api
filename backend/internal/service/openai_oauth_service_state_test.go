...
func TestOpenAIOAuthService_ExchangeCode_StateRequired(t *testing.T) {
	...
	require.NoError(t, svc.sessionStore.Put(context.Background(), "sid", &openai.OAuthSession{
...
func TestOpenAIOAuthService_ExchangeCode_StateMatch(t *testing.T) {
	...
	_, ok, _, err := svc.sessionStore.Take(context.Background(), "sid", "expected-state")
	require.NoError(t, err)
	require.False(t, ok)
}
