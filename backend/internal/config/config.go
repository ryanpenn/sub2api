...
type ServerConfig struct {
	...
	ShutdownTimeout          int       `mapstructure:"shutdown_timeout"`      // 排空与优雅关闭窗口（秒）
...
func setDefaults() {
	...
	viper.SetDefault("server.shutdown_timeout", 40)
...
func (c *Config) Validate() error {
	...
	if c.Server.ShutdownTimeout < 1 || c.Server.ShutdownTimeout > 45 {
		return fmt.Errorf("server.shutdown_timeout must be between 1 and 45 seconds")
	}
